...
let RecentlyPlayedJob = class RecentlyPlayedJob {
    ...
    async saveCashBacks(user, rescues, recently, campaign, questSpotifyPlaylist) {
        ...
        const userQuestSpotifySavePromise = this.userQuestSpotifyRepository.save(userQuestSpotifySave);
        const userQuestSpotifyUpdatePromise = [];
        userQuestSpotifyUpdate.forEach((uqs) => {
            userQuestSpotifyUpdatePromise.push(this.userQuestSpotifyRepository.update(uqs.id, uqs));
        });
        try {
            await Promise.all([
                statementsPromise,
